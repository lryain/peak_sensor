...
unsigned long lastTempUpdate = 0;             // Last time temp vas updated from controller (every 15min loop)
...
String options[] = {"TV (Belys)     ", "Mat (Belys)    ", "Dammsug        ", "Sovrumslampor  ", "Godnatt/Larma  "};
...
void setup() {
  ...
  if (newTempAvailable == true) {
    Serial.println("Got temp from init");
  ...
  mydisp.setColor(1);
  mydisp.setFont(fonts[1]);
  mydisp.print("    NER  /  OK  /  UPP"); // Buttons
...
void loop() {
  ...
  if (!sending) {
    ...
    if (!newTempAvailable && lastTempUpdate + 900000UL < now) {
      Serial.println("Updating temp/LCD");
...
void getTemp() {
  Serial.println("Sending AC-Protection-Temp");
